...
| Rewards grid | Open Rewards mid-game | 6 cards, aspect-true frames, scroll when needed, `Rewards:` copy + live influence for time rewards | Claim only when `n >= goal` |
...
- `phaser/src/scenes/PreloadScene.ts` — reload `mana-bar` / scene bg if used; scrollbar art
...
- [x] `phaser/src/scenes/play/rewards/{RewardCard,RewardsPanel}.ts` + `ui/ScrollList.ts` -- 6 cards (Helper, Clicker, Video, Earn Rewards, Login, Story), aspect-true frames, scroll+thumb, `Rewards:` copy + live influence amounts, projections stub progress -- R1–R6
...
- [x] `phaser/src/scenes/PreloadScene.ts` + `play/map/MapView.ts` -- use or drop `Scene.png` intentionally -- M3
...
- Given Rewards open with overflow progress, when viewing, then six cards scroll with aspect-true frames and Unity-like `Rewards:` copy
- Given projections stub used enough times, when Receive is ready, then claim grants ~10h influence and goal doubles
...
**Projections stub:** Add an in-card control (e.g. tap icon / “Watch”) that increments `videoCount` by 1 with a short toast — no ad network. Receive still claims the reward when `videoCount >= videoGoal`.

**Portal rehome:** After story unlock, expose portal region travel on Map (or Outlook) via existing toast/button pattern — not inside Settings.
